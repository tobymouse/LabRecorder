...
- **插入逻辑**：
  ...
  - 插入操作影响该样本的所有平台×条件×项目（每个单元格数组都在指定位置 splice 插入 `{result:null, note:''}`）
- **筛选兼容**：`dataRound` 已考虑筛选后的排序，插入位置符合显示顺序
- **样式实现**：复用 `#cellDropdown` 的 `.cd-item` 样式，`#addRoundDropdown` 为新增下拉容器
- **智能定位**：下拉菜单位置自动调整，避免超出屏幕边界（底部或右侧超出时自动换方向）
- **全局状态**：使用 `_addRoundCtx = { ci, ic, dataRound }` 存储当前操作上下文
