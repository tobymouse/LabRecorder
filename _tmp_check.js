...
function renderTable() {
  ...
  visiblePlatInfos.forEach(({ pl, plIdx, plConds, plItems, visibleCondIdxs, visibleItemIdxs }) => {
    visibleCondIdxs.forEach(cdi => {
      ...
      const noteIcon = hasCondNote ? `<span style="font-size:9px;margin-left:2px;opacity:.7;cursor:pointer;" onclick="event.stopPropagation();openConditionNoteModal(${plIdx},${cdi})" title="点击编辑条件备注">✎</span>` : '';
      html += `<th style="font-size:12px;min-width:68px;background:#f8fafc;cursor:pointer;" onclick="openConditionNoteModal(${plIdx},${cdi})" title="点击编辑条件备注">总结${noteIcon}</th>`;
      condColIdx++;
    });
  });
  if (noFilter) {
      ...
      for (let round = 0; round < rounds; round++) {
        ...
        visiblePlatInfos.forEach(({ pli, plItems, visibleCondIdxs, visibleItemIdxs }) => {
          ...
          visibleCondIdxs.forEach((cdi) => {
            ...
            // 收集该轮次所有项目的备注
            const sumNotes = visibleItemIdxs.map(ii => {
              const cell = getCell(ci, ic, pli, cdi, ii);
              const entry = cell[dataRound];
              return (entry && entry.note) ? `${plItems[ii]}: ${entry.note}` : null;
            }).filter(n => n !== null);
            const hasSumNotes = sumNotes.length > 0;
            const sumNoteIcon = hasSumNotes ? `<span style="font-size:9px;margin-left:2px;opacity:.7;" title="有${sumNotes.length}条备注">✎</span>` : '';
            html += `<td class="summary-cell-col" onclick="openSummaryNoteModal(${ci},${ic},${pli},${cdi},${dataRound})" title="点击查看/编辑总结备注">
        ...
        if (noFilter) {
          ...
          const hasRoundNote = visiblePlatInfos.some(({ pli, plItems, visibleCondIdxs }) => {
            return visibleCondIdxs.some(cdi => {
              return plItems.some((_, ii) => {
                const cell = getCell(ci, ic, pli, cdi, ii);
                const entry = cell[dataRound];
                return entry && entry.note && entry.note.trim() !== '';
              });
            });
          });
          const btnText = `操作${hasRoundNote ? ' ✎' : ''}`;
          const btnTitle = hasRoundNote ? '点击打开操作菜单' : '点击打开操作菜单';

          html += `<td style="vertical-align:middle;text-align:center;padding:2px 3px;">
            <span class="add-round-btn" onclick="openAddRoundDropdown(event,${ci},${ic},${dataRound})" title="${btnTitle}">${btnText}</span>
          </td>`;
          html += `<td style="vertical-align:middle;text-align:center;padding:2px 3px;">
...
//  次操作功能
// ─────────────────────────────────────────────
function openAddRoundDropdown(e, ci, ic, dataRound) {
  e.stopPropagation();
  _addRoundCtx = { ci, ic, dataRound };

  // 检查该次是否有备注
  const platforms = exp.platforms || [{ name: '默认平台', items: exp.items || [], conditions: exp.conditions || [] }];
  const hasNote = platforms.some((_, pli) => {
    const plItems = platforms[pli].items || [];
    const plConds = platforms[pli].conditions || exp.conditions || [];
    return plConds.some((_, cdi) => {
      return plItems.some((_, ii) => {
        const cell = getCell(ci, ic, pli, cdi, ii);
        const entry = cell[dataRound];
        return entry && entry.note && entry.note.trim() !== '';
...
function moveRound(direction) {
  ...
  // 检查边界条件
  if (direction === 'up' && dataRound === 0) {
    alert('已经是第一次了，不能上移！');
    closeAddRoundDropdown();
    return;
  }
  if (direction === 'down' && dataRound >= maxRounds - 1) {
    alert('已经是最后一次了，不能下移！');
    closeAddRoundDropdown();
    return;
  }

  const targetRound = direction === 'up' ? dataRound - 1 : dataRound + 1;
  ...
  platforms.forEach((_, pli) => {
    ...
    plConds.forEach((_, cdi) => {
      plItems.forEach((_, ii) => {
        const cell = getCell(ci, ic, pli, cdi, ii);
        // 交换位置
        [cell[dataRound], cell[targetRound]] = [cell[targetRound], cell[dataRound]];
        setCell(ci, ic, pli, cdi, ii, cell);
      });
    });
  });
...
function openRoundNoteModal(ci, ic, round) {
  ...
  platforms.forEach((pl, pli) => {
    ...
    plConds.forEach((cond, cdi) => {
      plItems.forEach((item, ii) => {
        const cell = getCell(ci, ic, pli, cdi, ii);
        const entry = cell[round];
        if (entry && entry.note && entry.note.trim() !== '') {
          notes.push(`【${pl.name} | ${cond} | ${item}】\n${entry.note}`);
        }
      });
    });
  });

  document.getElementById('roundNoteInput').value = notes.join('\n\n') || '';
...
function saveRoundNote() {
  ...
  platforms.forEach((_, pli) => {
    ...
    plConds.forEach((_, cdi) => {
      plItems.forEach((_, ii) => {
        const cell = getCell(_roundNoteCi, _roundNoteIc, pli, cdi, ii);
        const entry = cell[_roundNoteRound];
        ...
        setCell(_roundNoteCi, _roundNoteIc, pli, cdi, ii, cell);
      });
    });
  });
...
function makeRmTinyMCE(containerId, placeholder, pendingKey, fmtFn, toolbar, height) {
  ...
  // 定义默认工具栏（包含合并功能）
  const defaultToolbar = 'undo redo | formatselect | bold italic underline strikethrough | bullist numlist | link image | removeformat';
...
function noteGetOrInitTinyMCE() {
  ...
  tinymce.init({
    ...
    menubar: false,
    plugins: 'link image lists',
    toolbar: 'undo redo | bold italic underline | bullist numlist checkbox | separator date | link image | removeformat',
    branding: false,
    skin_url: 'https://cdn.jsdelivr.net/npm/tinymce@5.10.9/skins/ui/oxide',
    content_css: 'https://cdn.jsdelivr.net/npm/tinymce@5.10.9/skins/content/default/content.min.css',
...
//  总结备注
// ─────────────────────────────────────────────
let _summaryNoteQuill = null;
let _summaryNoteContext = null; // { ci, ic, pli, cdi, round }

function openSummaryNoteModal(ci, ic, pli, cdi, round) {
  _summaryNoteContext = { ci, ic, pli, cdi, round };

  const categoryName = exp.categories[ci] || `分类${ci+1}`;
  const platforms = exp.platforms || [{ name: '默认平台', items: exp.items || [], conditions: exp.conditions || [] }];
  const plat = platforms[pli];
  const cond = plat.conditions ? plat.conditions[cdi] : exp.conditions[cdi];
  document.getElementById('summaryNoteTitle').textContent = `📝 总结备注 - ${categoryName} #${ic+1} ${plat.name} | ${cond} 第${round+1}次`;

  // 收集该轮次所有项目的备注
  const notes = [];
  (plat.items || []).forEach((item, ii) => {
    const cell = getCell(ci, ic, pli, cdi, ii);
    const entry = cell[round];
    if (entry && entry.note) {
      notes.push(`<div style="margin-bottom:8px;"><strong>${item}:</strong> ${entry.note}</div>`);
    }
  });

  // 初始化 Quill 编辑器
  const container = document.getElementById('summaryNoteQuillContainer');
  container.innerHTML = '';
  ...
  _summaryNoteQuill = new Quill(container, {
    theme: 'snow',
    placeholder: '请输入总结备注...',
    modules: {
      toolbar: {
  ...
  // 设置内容
  if (notes.length > 0) {
    _summaryNoteQuill.root.innerHTML = notes.join('');
  }

  document.getElementById('summaryNoteModal').classList.add('show');
}

function saveSummaryNote() {
  if (!_summaryNoteContext) return;
  const { ci, ic, pli, cdi, round } = _summaryNoteContext;

  if (!_summaryNoteQuill) return;

  const html = _summaryNoteQuill.root.innerHTML;
  const text = _summaryNoteQuill.getText().trim();

  // 将备注拆分到各个项目（简单处理：按行拆分，每行一个项目）
  const platforms = exp.platforms || [{ name: '默认平台', items: exp.items || [], conditions: exp.conditions || [] }];
  const plat = platforms[pli];

  if (text) {
    // 如果有内容，尝试解析并分配到各个项目
    const lines = _summaryNoteQuill.root.querySelectorAll('div, p, li');
    lines.forEach((line, idx) => {
      ...
      if (match) {
        ...
        const ii = plat.items.indexOf(itemName);
        if (ii >= 0) {
          const cell = getCell(ci, ic, pli, cdi, ii);
          if (!cell[round]) cell[round] = { result: null, note: '' };
          cell[round].note = noteText;
        }
      }
    });
  }

  closeModal('summaryNoteModal');
  saveStore();
  renderTable();
}
...
function loadRecord(id) {
  ...
  // 换实验室提示
  const recLab = rec.name?.split('-')[0]?.trim();
  if (_currentLab && recLab && recLab !== _currentLab) {
    if (!confirm(`即将切换实验室：\n从 "${_currentLab}" 切换到 "${recLab}"\n\n确定要切换吗？`)) {
      return;
    }
  }
  _currentLab = recLab;
...
function exportExcel() {
  ...
  // 通过率总计行
  const S_TOTAL_LABEL = { font: { bold: true, color: { rgb: '1E3A5F' }, sz: 10 }, fill: { patternType: 'solid', fgColor: { rgb: 'BFDBFE' } }, alignment: { horizontal: 'center', vertical: 'center' }, border: BORDER };
  const S_TOTAL_PASS  = { font: { bold: true, color: { rgb: '15803D' }, sz: 10 }, fill: { patternType: 'solid', fgColor: { rgb: 'BBF7D0' } }, alignment: { horizontal: 'center', vertical: 'center' }, border: BORDER };
  const S_TOTAL_FAIL  = { font: { bold: true, color: { rgb: 'DC2626' }, sz: 10 }, fill: { patternType: 'solid', fgColor: { rgb: 'FED7AA' } }, alignment: { horizontal: 'center', vertical: 'center' }, border: BORDER };
  const S_TOTAL_EMPTY = { font: { bold: true, color: { rgb: '94A3B8' }, sz: 10 }, fill: { patternType: 'solid', fgColor: { rgb: 'F1F5F9' } }, alignment: { horizontal: 'center', vertical: 'center' }, border: BORDER };
...
function renderStats() {
  ...
  const allCondsUniq = [];
  const _condSeen = new Set();
  platforms.forEach(pl => (pl.conditions || exp.conditions || []).forEach(c => { if (!_condSeen.has(c)) { _condSeen.add(c); allCondsUniq.push(c); } }));
  msSetOptions('stat-cond', allCondsUniq.slice());
